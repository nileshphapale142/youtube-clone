                    ...
                    <div id={'dismissible'} className={'ytd-rich-grid-media'}>
                        <ytd-thumbnail rich-grid-thumbnail use-hovered-property width={'9999'}
                            ...
                            <a id={'thumbnail'} className={'yt-simple-endpoint ytd-thumbnail'}
                               aria-hidden={"true"} tabIndex={-1} rel={'null'} href={'/watch'}>
...
function Contents(props) {
    ...
    useEffect(() => {
        fetch('http://localhost:3001/')
            .then(response => response.json())
            .then(body => setYtVideos(body))
    }, [])

    let rowVideos = []
    let n = ytVideos.length

    for (let i = 0; i < n / 3;  i ++) {
        rowVideos.push(ytVideos.slice(3 * i, Math.min(3 * i + 3, n)))
...
export default function VideoBrowse() {
    return (
        <ytd-browse className={'ytd-page-manager'} rounded-container darker-dark-theme page-subtype={'home'}
                    rich-grid-watch-status={'inactive'} role={'main'} amsterdam has-sidebar
                    guide-persistent-and-visible>
            <ytd-two-column-browse-results-renderer className={'ytd-browse grid grid-disabled'} page-subtype={'home'}
                                                    disabled-grid-state-aware style={{touchAction: 'pan-down'}}>
                <div id={'primary'} className={'ytd-two-column-browse-results-renderer'}>
                    <ytd-rich-grid-renderer className={'ytd-two-column-browse-results-renderer'}
                                            style={{
